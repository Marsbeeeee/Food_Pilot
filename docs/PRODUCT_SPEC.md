...
任务 9：建设更贴近中国用户的真实食物数据底座

目标：

- 在现有首期中文知识库基础上，进一步形成面向中国用户的标准化食物目录，而不只是持续堆积零散条目。

下一阶段要求：

- 扩充高频中国场景覆盖：家常菜、早餐小吃、地方面食、外卖套餐、便利店食品、品牌饮品、轻食与健身餐。
- 强化标准化结构：为条目建立稳定 `food_id`、标准名、别名、份量提示、来源、更新时间与必要的组合菜拆分信息。
- 优先保证“命中稳定、来源可追溯、维护可持续”，再考虑是否迁移为独立运营数据库。

阶段边界：

- 当前阶段不承诺一次性建设完整全国菜品数据库。
- 当前阶段不引入高成本在线抓取与实时外部同步作为主路径。
