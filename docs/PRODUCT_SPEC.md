...
1. Insights 分析选择篮（已选条目）仍为前端 `localStorage`，换设备需重新勾选；历史分析无按日期回查 API，仅作缓存预填。日期选择器可满足查看历史需求，无需独立历史列表 UI。
2. 前端测试当前仅有少量 Node 测试文件，关键 UI 流程覆盖不足。
...
1. 前端自动化：缺少覆盖登录-提问-保存-Insights 的端到端/集成测试。
...
2. **Insights 服务化补全**（部分完成）
   ...
   - 待完成：分析选择篮后端同步；按日期回查 API。（历史列表 UI 已从 P0 移除，日期选择器足以满足查看历史需求）
3. **前端关键流程自动化测试**
