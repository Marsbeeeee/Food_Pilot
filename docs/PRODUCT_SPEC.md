...
#### 4.4.8 菜品图片自动补齐（下一阶段需求）

产品目标：

- 当用户向 Food Log 保存菜品时，如果该菜品能稳定命中标准化食物条目，系统可自动补齐与该菜品对应的图片。
- 自动补图的目标是提升 Food Log 的可读性与可回看性，不改变估算结果本身，也不把图片识别引入主保存链路。

设计原则：

- 自动补图依赖“标准化菜品匹配”，不直接基于自由文本做无约束搜图。
- 只有高置信命中标准条目时才自动补图；低置信度场景宁可留空，也不错误补图。
- 图片需要保留来源与授权元数据，继续沿用 `image / imageSource / imageLicense` 治理规则。
- 自动补齐图片属于“展示增强”，不应阻塞 Food Log 的主保存流程。
...
- 下一阶段如引入更大规模中国食物标准库、后台运营维护、图片映射关系与多来源同步，再评估是否拆分为独立表结构或独立数据库。
...
7. 图片能力当前已具备字段、治理与展示基础，但“按标准菜品自动补齐真实图片”尚未进入主保存流程。
...
> Food Pilot 当前是一款以聊天为主入口的营养助手。用户登录后可在 Chat 中获取餐食推荐或营养估算，把值得保留的结果显式保存到 Food Log，并在 Insights 中做日/周分析。Profile 用于个性化与过敏约束。当前核心闭环已成立，已具备首期中文食物知识增强；下一阶段重点优化方向集中在更贴近中国用户的标准化食物数据、Food Log 图片自动补齐能力、意图分流准确率和 Insights 趋势表达。
...
- 已将稳定标识提升为 `food_id` 语义，并补充 `scene_tags / region_tags` 等结构化扩展位，为后续持续扩容和图片映射提供统一主键基础。
...
任务 10：为 Food Log 增加标准菜品图片自动补齐能力

目标：

- 当保存项成功命中标准化菜品时，自动补齐对应图片，提升 Food Log 列表与详情的识别效率和回看体验。

下一阶段要求：

- 图片补齐建立在“标准菜品映射”之上，而不是自由文本直连搜图。
- 自动补图只在高置信度命中时执行，不阻塞保存主链路。
- 图片资源需带来源与授权信息，并继续走现有图片治理规则。
- 支持后续人工校正或用户自传图片覆盖系统默认图。

阶段边界：

- 当前阶段不做开放式网络抓图后直接落库。
- 当前阶段不把补图能力扩展为图片识别主入口。
