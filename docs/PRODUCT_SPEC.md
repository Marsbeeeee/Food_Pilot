...
- 已梳理并打通 Food Log 列表接口与前端筛选交互，在不破坏现有接口兼容性的前提下新增 `sourceType` 与 `hasImage` 筛选参数。
- 已在 Explorer 增加更明确的搜索入口与高频筛选组合：关键词搜索、来源筛选、是否有图筛选、时间范围筛选、排序切换与一键清空筛选。
- 关键词、时间范围、排序、来源与图片条件已支持稳定组合使用，且默认仍保持原有“最新优先”行为。
- Food Log 保存、编辑、软删除恢复与来源回跳链路保持不变。
- 已补充后端仓库/API 组合筛选测试与前端筛选逻辑单测，相关测试通过。
