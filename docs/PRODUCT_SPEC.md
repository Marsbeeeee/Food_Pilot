...
任务 7-B（未完成）：扩充中文食物知识覆盖（数据层）

背景：
当前知识库已覆盖高频场景，但条目规模仍偏小，长尾菜品、地区化叫法、烹饪变体覆盖不足。随着真实查询多样化提升，现有数据规模会成为命中率与解释质量的主要瓶颈。

要求：

- 扩展食物条目与别名覆盖，优先补齐高频长尾（地方菜、外卖常见菜、品牌饮品变体）。
- 强化结构化字段一致性：`canonical_name / aliases / nutrition_per_100g / source_ids / updated_at` 必填与格式校验。
- 为新增条目补齐可追溯来源，确保引用链路稳定输出 `food_name + source_id + source_name`。
- 迭代数据版本治理：每次增量更新都需提升 `version` 并记录变更摘要。

验收标准：

- 数据规模达到下一阶段目标（建议不少于 60 条高频与次高频中文条目）。
- `scripts/validate_food_kb_rag.py` 全量通过，且覆盖率阈值提升到不低于 85%。
- 新增条目无重复 `canonical_name`，关键字段缺失率为 0。
- 抽样查询可稳定返回可追溯引用，且不破坏现有链路测试通过率。
