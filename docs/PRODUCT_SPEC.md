...
- 数据规模已达到并超过下一阶段目标：`67 / 60+`。
- `scripts/validate_food_kb_rag.py` 已通过，检索覆盖率为 `100% (20/20)`，高于 `85%` 门槛。
- 新增条目无重复 `canonical_name`，关键字段缺失率为 `0`。
- 相关回归验证已通过：`backend/tests/test_food_knowledge_retrieval.py`、`backend/tests/test_estimate_food_knowledge_integration.py`、`backend/tests/test_recommendation_food_knowledge_integration.py` 共 `9` 项测试全部通过。

任务 7-C（未完成）：提升检索精度与评估可观测性（算法与质量层）

背景：
当前检索采用规则匹配，稳定但上限有限；在同义词密集、组合菜名、对比问法中，仍存在召回与排序不稳定风险。缺少持续可观测的评估面板，也会放大回归风险。

要求：

- 优化检索打分策略：提升组合菜名、同义词、近义表达下的召回与排序稳定性。
- 建立回归评估集：补充真实问法样本，覆盖估算、推荐、对比、替换等场景。
- 引入质量监控指标：命中率、Top-1 准确率、引用完整率、无命中率，并形成可追踪基线。
- 保持失败降级和开关策略不变，避免引入主链路可用性回归。

验收标准：

- 在固定评估集上，Top-1 命中率与整体召回率相对当前基线有可量化提升（建议提升不少于 10%）。
- 无命中率下降且引用完整率保持 100%（命中样本）。
- 新增评估与回归测试纳入发布门禁，脚本与测试可在本地一键执行。
- `estimate / recommendation / text` 三条链路回归测试全部通过，且未出现契约字段回退。
...
截至 2026-03-23，Food Pilot 已经不是纯 demo。
