...
These three checks are the default release gate and are expected to pass stably in CI and local dev.
...
- Retrieval validation now checks dataset health, fixed eval-set hit rate, Top-1 accuracy, Top-K recall, citation completeness, no-hit rate, and baseline regressions.
- `backend/data/food_kb_quality_baseline.json` is the committed retrieval baseline snapshot.
...
Behavior:
- Always runs backend tests, retrieval quality validation, and frontend unit tests.
